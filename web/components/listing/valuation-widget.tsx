    ...
    <div className="rounded-xl border border-border bg-muted px-4 py-4 space-y-4">
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <div className="rounded-lg bg-white border border-neutral-100 px-3 py-2.5 text-center">
          <p className="text-[10px] text-neutral-400 uppercase tracking-wide mb-0.5">€/m² medio</p>
          <p className="text-sm font-bold text-neutral-900">
            {currentPricePerSqm ? `€${currentPricePerSqm.toLocaleString('it-IT')}` : '—'}
          </p>
          {avgPricePerSqm > 0 && (
            <p className="text-[10px] text-neutral-400">zona: €{avgPricePerSqm.toLocaleString('it-IT')}</p>
          )}
        </div>
      </div>
      ...
      <div className="space-y-2">
        <div className="flex items-center gap-1.5 mb-1">
          ...
          <p className="text-[11px] text-neutral-400">{comparables.length} vendite simili archiviate</p>
        </div>

        {/* Current listing bar */}
        <div className="flex items-center gap-2">
          <div className="w-24 shrink-0 truncate text-[11px] text-neutral-500">Questo immobile</div>
          <div className="flex-1 rounded-full bg-neutral-200 h-2 relative overflow-hidden">
            <div
              className="absolute left-0 top-0 h-full rounded-full bg-neutral-800"
              style={{ width: `${(currentPrice / maxPrice) * 100}%` }}
            />
          </div>
          <div className="w-20 shrink-0 text-right text-[11px] font-medium text-neutral-700">
        ...
        {comparables.map((l, i) => (
          <div key={i} className="flex items-center gap-2">
            <div className="w-24 shrink-0 truncate text-[11px] text-neutral-400">{l.address.split(',')[0]}</div>
            <div className="flex-1 rounded-full bg-neutral-200 h-2 relative overflow-hidden">
              <div
            ...
            <div className="w-20 shrink-0 text-right text-[11px] text-neutral-500">
