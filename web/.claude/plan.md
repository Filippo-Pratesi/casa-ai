...
## Product Plan v4.0 (Reviewed 2026-03-18)
...
2. **iOS App (React Native + Expo)** — Planned for Sprint F
...
| **Auto buyer matching** (listing detail: matched count + notify button) | ✅ Done |
| **Compatible listings on contact** (buyer contacts see matching listings) | ✅ Done |
| **Google Calendar sync** (OAuth2, push/pull, in settings/integrazioni) | ✅ Done |
| **MLS basic** (Condividi con rete toggle, read-only for other workspaces) | ✅ Done |
| **Listing stats** (mocked: views, shares, portal clicks) | ✅ Done |
| **Export contacts CSV** (admin, all fields) | ✅ Done |
| **Import contacts CSV** (5-row preview, duplicate skip, template download) | ✅ Done |
| **Security audit** (SECURITY_AUDIT.md: 2 critical, 3 high, 4 medium) | ✅ Done |
| **Security fixes** (workspace_id on appointments + campaign attachments) | ✅ Done |
| **Contacts: filtri avanzati** (tipo, budget, città, stanze; card/lista toggle) | ✅ Done |
| **Contacts: icona WhatsApp** (SVG ufficiale) | ✅ Done |
| **Team: filtri dinamici** (per agente, periodo personalizzato) | ✅ Done |
| **Thank-you email automatica** (AI bozza via Gemini su venduto, in Campagne) | ✅ Done |
| **i18n IT/EN** (language switcher sidebar, tutte le pagine) | ✅ Done |
| **To Do system** (priorità, scadenze, assegnazione, badge sidebar) | ✅ Done |
| **Fix AI generation bug** (admin client per profile lookup) | ✅ Done |
| **Dark mode toggle** (sidebar, persisted) | ✅ Done |
| **Command palette** (Ctrl+K quick search/navigation) | ✅ Done |
...
| Auto Matching display (buyer prefs → listings count on listing detail) | ✅ Done |
...
### Priority 3c — Sprint F (Mobile App) ❌ NOT STARTED
...
### Security Sprint (from SECURITY_AUDIT.md) ❌ PENDING
...
### ✅ Sprint E — MOSTLY COMPLETE (Workflow & i18n)
...
- [ ] **Lead nurturing sequences** — Timed follow-up email/WhatsApp per contact (XL — deferred)
...
| **Dark mode** | ✅ most | ✅ | ✅ |
| **Command palette (Ctrl+K)** | ✅ some | ✅ | ✅ |
| Agency website | ✅ Getrix, AGIM, Cometa | ❌ | Sprint F / v3 |
| iOS app | ✅ Gestim, Getrix, AGIM | ❌ | Sprint F |
| Lead nurturing sequences | ❌ none with AI | ❌ | Sprint F |
