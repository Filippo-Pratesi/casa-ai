...
const ELIGIBLE_STAGES = ['conosciuto', 'incarico', 'disponibile']

// GET /api/banca-dati/match?property_id=...
// Without ?ai=1 → computes deterministic score on-demand from properties table (banca dati page)
...
export async function GET(req: NextRequest) {
  ...
  // --- Deterministic mode: compute on-demand from properties table (banca dati) ---
  ...
  // Only eligible stages
  if (!prop.stage || !ELIGIBLE_STAGES.includes(prop.stage)) {
    return NextResponse.json({ matches: [], status: 'not_eligible', stage: prop.stage })
  }
  ...
  // Determine which contact types to query based on transaction_type
