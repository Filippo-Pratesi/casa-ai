...
export async function GET(_req: NextRequest, { params }: Params) {
  ...
  if (error || !data) return NextResponse.json({ error: 'Proposta non trovata' }, { status: 404 })
...
export async function PATCH(req: NextRequest, { params }: Params) {
  ...
  const { id: _id, workspace_id: _ws, agent_id: _ag, numero_proposta: _np, anno: _anno, progressivo: _prog, ...updateable } = body
  void _id; void _ws; void _ag; void _np; void _anno; void _prog
...
// DELETE /api/proposals/[id] — only bozza proposals
export async function DELETE(_req: NextRequest, { params }: Params) {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const { error } = await (admin as any).from('proposals').delete().eq('id', id)
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })
