...
// GET /api/proposals/next-number?anno=2026
export async function GET(req: NextRequest) {
  ...
  const { data } = await (admin as any).rpc('next_proposal_number', {
  ...
  return NextResponse.json({ anno, progressivo, numero_proposta })
}
