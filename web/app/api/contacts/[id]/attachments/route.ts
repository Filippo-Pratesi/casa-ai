...
export async function POST(
  ...
  const storagePath = `${profile.workspace_id}/contacts/${id}/${Date.now()}-${Math.random().toString(36).slice(2)}.${ext}`
...
export async function DELETE(
  ...
  await admin.storage.from('contact-docs').remove([att.storage_path])
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  await (admin as any).from('contact_attachments').delete().eq('id', attachmentId)
